...
const TwoFactorAuthGeneration = ({ twoFA_setup }) => {
  ...
  const handleVerifyCode = async () => {
    try {
      const response = await api.post("/auth/verify2fa/", { email:"raghu@gmail.com",token: verificationCode });
      ...
      toast.error(error.response?.data?.message || "Failed to verify the code.");
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-stone-50">
      <Card className="bg-white rounded-xl shadow-lg p-6 max-w-md">
        <CardHeader>
          <CardTitle className="text-center text-xl">Enable Two-Factor Authentication</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            <div>
              <h2 className="font-semibold">Step 1: Set Up Your Authenticator App</h2>
              <p className="text-sm text-gray-600">
                Open your authenticator app (e.g., Google Authenticator, Authy) scan the QR code 
              </p>
              {loading ? (
                <Loader /> // Show the loader while QR code is being generated
              ) : (
                <img src={qrCode} alt="QR Code" className="mx-auto mt-4 w-48 h-48" />
              )}
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Enter the 6-digit code generated by your authenticator app below.
              </p>
              <Input
                type="text"
                inputMode="numeric"
